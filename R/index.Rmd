...
dato <- merge_opp@data %>% 
  ...
  summarize(n=n()) %>% 
  cast(countyfips ~ c5_COI_nat)

datn <- dato %>% # reshape for plotting
  mutate(Counties = factor(countyfips)) %>% 
  gather(variable, value, -Counties) %>% 
  slice(6:n())

datn %>% # stacked bar chart 
  mutate(value=as.numeric(value)) %>% 
  ggplot(aes(x = Counties,
             y = value,
             fill = forcats::fct_rev(variable))) +  
...
schools$ctny_cname <- paste0(schools$STCNY, schools$census_tract) # Census county + abv tract

# use str as sub for NA value in ctnycname identifier
...
svi_opp$ctny_cname <- paste0(svi_opp$STCNTY, svi_opp$NAME.y) # concat to join schools

schools %>% select(system_code, community_district, borough_block_lot, ctny_cname) %>% arrange(ctny_cname)
n_distinct(schools$ctny_cname)

svi_opp %>% group_by(ctny_cname) %>% filter(n()>1)
n_distinct(svi_opp$ctny_cname)

a <- svi_opp$ctny_cname
a <- unique(a)
b <- schools$ctny_cname
b <- unique(b)
                                              #no period   #period  
length(intersect(a,b)) # what's in common -    18 values    180
length(setdiff(a,b)) # what's different in a - 2148 values  1984
length(setdiff(b,a)) # what's different in b - 954 values   791

c <- svi_opp$countyfips
c <- unique(c)
d <- schools$STCNY
d <- unique(d)

length(intersect(c,d)) # what's in common  
setdiff(c,d) # what's different in c 
length(setdiff(d,c)) # what's different in d 
